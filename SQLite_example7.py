...
def main():
	...
	pointsWithinGBB = findPointsWithinAGeographicBoundingBox("OSM_BritishIsles_Amenities.sqlite",51.5172,-0.1464,51.48383,-0.0808)
	
	# let's write the results to a CSV file - we will just write the coordinates
	outputCSVFile = open("PointsWithinGBB.csv","w")
...
# Notice that we do not make any checks on the type of amenity we are looking for - any amenity

def findPointsWithinAGeographicBoundingBox(databaseName,topLat,leftLong,bottomLat,rightLong):
	queryResults = None
	...
	try: 
		with con:
			...
			cur.execute('SELECT * FROM amenities where latitude <= ?  and latitude >= ? and longitude >= ? and longitude <= ?',(topLat,bottomLat,leftLong,rightLong,))
