...
def main():
	...
	outputCSVFile = open("Pubs_BritanIreland.csv","w")
...
def doAnSQLiteQuerySpecificAmenityType(databaseName,amenityType):
	...
	try: 
		with con:
			...
			cur = con.cursor()
			cur.execute('SELECT AmenityName,Amenity,Longitude,Latitude from amenities where amenity = ? and length(AmenityName) > 0',(amenityType,))
