...
def main(): , 
	...
	myUTMCoordinate = utm.from_latlon(53.3472577, -6.2590866)
	myUTMCoordinateX = myUTMCoordinate[0]
	myUTMCoordinateY = myUTMCoordinate[1]
	distanceBufferKM = 4 ## remember this is the square of what we want. 
	
	pointsWithinBuffer = findPointsWithinDistanceBuffer("OSM_BritishIsles_Amenities.sqlite",myUTMCoordinateX,myUTMCoordinateY,distanceBufferKM)
	
	# let's write the results to a CSV file - 
	outputCSVFile = open("PointsWithinBuffer.csv","w")
...
# We provide 3 variables include the databaseName
# we provide our UTMX coordinate, UTMY coordinate, and the distance squared of our buffer. 

def findPointsWithinDistanceBuffer(databaseName,myUTMX, myUTMY, myDistanceSquared):
	queryResults = None
	...
	try: 
		with con:
			...
			cur.execute('SELECT * FROM amenities where ( (? -UTMX)*(? -UTMX) + (? - UTMY)* (? - UTMY))/100000 <= ?',(myUTMX,myUTMX,myUTMY,myUTMY,myDistanceSquared,))
