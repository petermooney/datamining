...
def main():
	resultsAmenitiesWithNames = doAnSQLiteQuery("OSM_BritishIsles_Amenities.sqlite")

	for amenity in resultsAmenitiesWithNames:
...
#Query purpose: This query will return every amenity in the database where the name of the amenity (AmenityName)
# is not an empty string - so it has string length > 0 (there are some characters)

def doAnSQLiteQuery(databaseName):
	...
	try: 
		with con:
			...
			cur.execute("SELECT AmenityName,Amenity from amenities where length(AmenityName) > 0")
